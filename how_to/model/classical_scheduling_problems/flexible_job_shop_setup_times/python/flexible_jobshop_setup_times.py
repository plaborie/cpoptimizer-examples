...
s = [ sequence_var( [y[i][j][u] for i in N for j in range(O[i]) for u,a in enumerate(J[i][j]) if a[0]==k],
                    [a[2]       for i in N for j in range(O[i]) for u,a in enumerate(J[i][j]) if a[0]==k] ) for k in M]
model.add(
 # Minimize makespan
 ...
 # Machines can only process one operation at a time with a sequence-dependent setup time
 [ no_overlap(s[k],S)                     for k in M]
)
